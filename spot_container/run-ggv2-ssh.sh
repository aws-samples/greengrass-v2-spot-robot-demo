...
then
  ssh -p $PORT $HOST_STRING "docker run -d --name aws-iot-greengrass-v2 --restart=always --gpus all --privileged --entrypoint /greengrass-entrypoint.sh -v /opt/payload_credentials:/payload_credentials -v /home/spot/ggv2:/greengrass/v2 -e AWS_REGION=${AWS_DEFAULT_REGION} -e THING_NAME=${THING_NAME} -e THING_GROUP_NAME=spot_group -e AWS_ACCESS_KEY_ID=${AWS_ACCESS_KEY_ID} -e AWS_SECRET_ACCESS_KEY=${AWS_SECRET_ACCESS_KEY} --net=host ${IMAGE}"
  ...
  ssh -p $PORT $HOST_STRING "docker run -d --name aws-iot-greengrass-v2 --restart=always --privileged --entrypoint /greengrass-entrypoint.sh -v /opt/payload_credentials:/payload_credentials -v /home/spot/ggv2:/greengrass/v2 -e AWS_REGION=${AWS_DEFAULT_REGION} -e THING_NAME=${THING_NAME} -e THING_GROUP_NAME=spot_group -e AWS_ACCESS_KEY_ID=${AWS_ACCESS_KEY_ID} -e AWS_SECRET_ACCESS_KEY=${AWS_SECRET_ACCESS_KEY} --net=host ${IMAGE}"
