...
class NetworkComputeBrideAdapter:
    ...
    def get_capabilities(self):
        ...
        while True:
            try:
                server_data = (
                    network_compute_bridge_pb2.NetworkComputeServerConfiguration(
                ...
                response = (
                    self._network_compute_bridge_client.list_available_models_command(
                        list_req
                    )
                )
                break
    ...
    def get_network_compute_data(self, request, store_helper):
        ...
        try:
            ...
            network_compute_bridge_metadata = request.metadata.data[
                "network_compute_bridge"
            ]
            response = self._request_data(
                request, network_compute_bridge_metadata, store_helper
            )

            _LOGGER.info("NCB RESPONSE:")
            ...
            for capture in request.acquisition_requests.data_captures:
                ...
                if capture.name == kCapabilityImage:
                    ...
                    detection_metadata.data.update(
                        json_format.MessageToDict(detection_info)
                    )
...
def make_service(robot, network_compute_bridge_worker_name, logger=None):
    ...
    return DataAcquisitionPluginService(
        robot, capabilities, adapter.get_network_compute_data
    )


def run_service(
    bosdyn_sdk_robot, port, network_compute_bridge_worker_name, logger=None
):
    ...
    service_servicer = make_service(
        bosdyn_sdk_robot, network_compute_bridge_worker_name, logger=logger
    )
...
def main():
    ...
    parser.add_argument(
        "-p",
        "--port",
        help=f"Server's port number, default: {default_port}",
        default=default_port,
    )
    parser.add_argument(
        "--on-spot-core",
        help="Use SpotCore GUID to register the server with Spot",
        action="store_true",
    )
    parser.add_argument(
        "--guid",
        help="Unique GUID of the payload, new one will be generated if none specified",
    )
    parser.add_argument(
        "--secret",
        help="Secret of the payload, new one will be generated if none specified",
    )
    ...
    service_runner = run_service(
        robot, options.port, options.worker_name, logger=_LOGGER
    )
